const products = [
  ...
  },
  {
    ...
    features: ["Intel Core i5-1235U", "", "8GB RAM"],
    inStock: true,
    warranty: "1 год",
    brand: "Microsoft",
  },
...
function showSearchResults(quiery) {
  //Используем фильтр для поиска товаров
  const seacrhResultsArray = products.filter(function (product) {
    const productName = product.name.toLowerCase();
    const searchQuiery = quiery.toLowerCase();
    const productDescription = product.description.toLowerCase();
    return (
      productName.includes(searchQuiery) ||
      productDescription.includes(searchQuiery)
    );
  });
  console.log("Найдено товаров", seacrhResultsArray.length);
  const limitedResults = seacrhResultsArray.slice(0, 5);
  displaySearchDropDown(limitedResults);
}
...
function displaySearchDropDown(results) {
  ...
  //Используем forEach для создания результатов
...
function hendleCategoryFilter(btn) {
...
function getFilteredProducts() {
  ...
  console.log("Текущий фильр:", currentFilter);
  console.log("Поисковый запрос:", searchTerm);
  //Используем метод filter для отбора товаров
  let filteredProductsValue = products.filter((product) => {
    //Проверка категории
    let matchesCategory = true;
    if (currentFilter !== "all") {
      matchesCategory = product.category === currentFilter;
    }
    //Проверка поиска
    let matchesSearch = true;
    if (searchTerm !== "") {
      const productName = product.name.toLowerCase();
      const productDescription = products.description.toLowerCase();
      const searchQuiery = searchTerm.toLowerCase();
      matchesSearch =
        productName.includes(searchQuiery) ||
        productDescription.includes(searchQuiery);
    }
  ...
  console.log("После фильтрации осталось:");
  const sortedProducts = sortedProducts(filteredProducts);
  return sortedProducts;
}
